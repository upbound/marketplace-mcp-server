...
func (s *Server) handleAuthenticate(ctx context.Context, req *MCPRequest, args map[string]interface{}) error {
	clientID := "marketplace-mcp-client"
	if id, ok := args["client_id"].(string); ok && id != "" {
		clientID = id
	}

	scopes := []string{"read:packages", "read:repositories"}
	if scopesArg, ok := args["scopes"].([]interface{}); ok {
		scopes = make([]string, len(scopesArg))
		for i, scope := range scopesArg {
	...
	// Create new auth manager with provided parameters
	s.authManager = auth.NewAuthManager(clientID, "", scopes)
