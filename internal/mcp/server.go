...
func NewServer(client *marketplace.Client) *Server {
	// Initialize auth manager with default OAuth config
	authManager := auth.NewAuthManager(
		"marketplace-mcp-client", // Default client ID
		"",                       // Client secret (if needed)
		[]string{"read:packages", "read:repositories"},
	)
...
func (s *Server) handleToolsList(req *MCPRequest) error {
	tools := []Tool{
		...
		},
		{
			...
			Description: "Get assets (CRDs, examples, docs) for a specific package version",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"account": map[string]interface{}{
					...
					"asset_type": map[string]interface{}{
						"type":        "string",
						"description": "Type of asset (crds, examples, docs, package)",
						"enum":        []string{"crds", "examples", "docs", "package"},
					},
				},
		...
		{
			Name:        "authenticate",
			Description: "Authenticate with Upbound to access private resources",
			InputSchema: map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					...
					"scopes": map[string]interface{}{
						...
						"default": []string{"read:packages", "read:repositories"},
					},
				},
			},
		},
